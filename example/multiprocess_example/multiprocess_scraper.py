...
if 'edison_patents.csv' not in os.listdir(path_to_data):
    with open(path_to_data + 'edison_patents.csv', 'w', newline='', encoding='UTF-8') as ofile:
...
if __name__ == "__main__":
    ...
    with poolcontext(processes=mp.cpu_count()-1, initializer=init, initargs=(l,)) as pool:
        pool.map(partial(single_process_scraper,
                         path_to_data_file=path_to_data + 'edison_patents.csv',
